...
//проверяем есть ли среди них удаляемый элемент, если есть - поылаем уведомление админу
...
/*// обработчик для votes_limit_users, ограничение голосований одного пользователя
 ...
 //получаем ID текущего пользователя
    ...
    if ($userid) {
			...
			$res = CIBlockElement::GetList(array(), $arFilter);
 			...
 			//если голосования не было, добавляем элемент инфоблока с соответствующими значениями свойств
		  if (!$voted) {
		    ...
		    while ($ob = $res->GetNext()) {
		      $usersId[] = $ob['VALUE'];
				...
				CIBlockElement::SetPropertyValuesEx($_POST["id"], $_POST["iblock-id"], array("USERS_ID" => $usersId));
				CIBlockElement::SetPropertyValues($_POST["id"], $_POST["iblock-id"], $_POST["rating"], "RATING");
